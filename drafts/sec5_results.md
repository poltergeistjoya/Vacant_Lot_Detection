...
- **Best DeepLabV3+ by F2:** Run 018 (ResNet-50, 5ch, 1024px, v2 mask, no warmup): val F2 0.405, test F2 0.385. Run 024 (with warmup) achieves higher validation IoU (0.193 vs. 0.154) but substantially lower F2 (val 0.295, test 0.197) because warmup shifts the model toward a precision-oriented decision boundary -- see Section 5.4.
...
| 018 | DLV3+ | RN50 | 5 | 1024 | v2 | **0.405** | 0.267 | **0.385** | **0.259** | Best F2 overall |
...
| Metric | DLV3+ 018 | UNet 030 | UNet 031 | DLV3+ 024 |
|--------|----------:|---------:|---------:|----------:|
| Architecture | DeepLabV3+ | UNet | UNet | DeepLabV3+ |
| Backbone | ResNet-50 | ResNet-34 | ResNet-34 | ResNet-50 |
| Channels | 5 | 5 | 4 | 5 |
| Patch size | 1024 | 512 | 512 | 1024 |
| Warmup | No | No | No | Yes |
| **Val F2** | **0.405** | 0.367 | 0.347 | 0.295 |
| Val F1 | 0.267 | 0.237 | 0.289 | 0.324 |
| Val Precision | 0.170 | 0.149 | 0.227 | 0.388 |
| Val Recall | 0.617 | 0.580 | 0.400 | 0.278 |
| Val AP | 0.185 | 0.156 | 0.228 | 0.273 |
| **Test F2** | **0.385** | **0.341** | 0.277 | 0.197 |
| Test F1 | **0.259** | 0.222 | 0.248 | 0.236 |
| Test Precision | 0.168 | 0.140 | 0.210 | 0.357 |
| Test Recall | 0.570 | 0.530 | 0.301 | 0.177 |
| Test AP | 0.168 | 0.145 | 0.180 | 0.217 |
| Test IoU† | 0.149 | 0.125 | **0.141** | 0.134 |
...
**F2 as primary metric.** Run 018 is the strongest model by both test F2 (0.385) and test F1 (0.259). Among UNet runs, run 030 achieves the highest test F2 (0.341). The high-F2 models share a common trait: high recall (0.53--0.62) at the cost of precision (0.14--0.17). For a screening application, this tradeoff is appropriate -- false positives can be discarded by a human reviewer, while missed vacant lots (false negatives) represent permanently lost opportunities.
...
**Summary.** For the screening use case this model is designed for, run 018 (DLV3+, ResNet-50, 5ch, 1024px, v2 mask) is the recommended model. Run 030 (UNet, ResNet-34, 5ch, 512px) is the best alternative if a UNet architecture is preferred for deployment simplicity.
...
**[PLACEHOLDER: F2-Priority Runs]** If additional runs complete with modified gradient norm clipping aimed at higher recall, results will be added here and to Appendix C.
